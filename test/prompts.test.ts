...
const primary = (...texts: string[]): DiffSummary[] => texts.map((text) => ({ priority: 'primary', text }))
...
describe('filename prompts', () => {
  test('quotes unusual filenames and requests a structured single message', () => {
    const prompt = buildFilenamesUser({ primary: ['src/quo"te\nname.ts', 'src/ünicode.ts'], lowPriority: [] }, 1, true)
...
describe('buildFinalSystem', () => {
  ...
  test('gitmoji asks for a leading emoji and orders it before the type', () => {
    const sys = buildFinalSystem(mergeConfig(DEFAULT_CONFIG, { gitmoji: true, conventionalCommits: true }))
...
describe('buildFinalUser with low-priority summaries', () => {
  const mixed = [...primary('fix null deref in parser'), ...low('regenerated skill docs', 'bumped lockfile')]
  ...
  test('multi-option wording is unchanged when there is a single group', () => {
    for (const user of [buildFinalUser(primary('x'), 3, false), buildFinalUser(primary('x'), 3, true)]) {
