...
describe('diffPaths', () => {
  ...
  test('preserves both rename paths alongside additions, deletions and binaries', () => {
    ...
    expect(diffPaths(diff)).toEqual(['old name.txt', 'new name.txt', 'added.txt', 'deleted.txt', 'image.png'])
  })
})
...
--- a/big.txt
+++ b/big.txt`
    const hunks = Array.from({ length: 6 }, (_, i) => `@@ -${i},1 +${i},1 @@\n-line ${i} old\n+line ${i} new`)
    const diff = `${header}\n${hunks.join('\n')}`
...
describe('sectionPaths', () => {
  ...
  test("resolves a header whose path itself contains ' b/'", () => {
    const tricky = ['diff --git a/sub b/file.txt b/sub b/file.txt', 'old mode 100644', 'new mode 100755'].join('\n')
  ...
  test('unquotes C-style quoted paths, including octal UTF-8 escapes', () => {
    ...
    const quotedMode = ['diff --git "a/tab\\there.txt" "b/tab\\there.txt"', 'old mode 100644', 'new mode 100755'].join(
      '\n'
    )
...
describe('applyIgnorePatterns', () => {
  ...
  test('returns the diff untouched when nothing matches', () => {
    const result = applyIgnorePatterns(`${codeFile}\n${vendorFile}`, createPathMatcher(['node_modules']))
  ...
  test('can empty the diff entirely - the caller decides what that means', () => {
    const result = applyIgnorePatterns(`${vendorFile}\n${lockFile}`, createPathMatcher(['vendor/**', 'bun.lock']))
