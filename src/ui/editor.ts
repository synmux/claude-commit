...
export async function editInEditor(initial: string): Promise<string> {
  ...
  try {
    ...
    throw new ClaudeCommitError(`Could not create a temporary file to edit the message: ${(err as Error).message}`)
  }
  try {
    ...
    throw new ClaudeCommitError(`Editing the commit message failed (${editor}): ${(err as Error).message}`)
  } finally {
...
export async function confirmCommit(): Promise<ConfirmChoice> {
  ...
  try {
    for (;;) {
      const answer = (await new Promise<string>((res) => rl.question('Commit this message? [Y/n/e] ', res)))
        .trim()
        .toLowerCase()
