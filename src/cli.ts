...
import { generateCommit, type IgnoreStats, type LowPriorityStats, type OllamaContextWindow } from './generate.ts'
import { commit, getRepoRoot, getStagedDiff, getStagedStat, isGitRepo, stageAll } from './git.ts'
...
export function buildProgram(): Command {
  ...
  program
    ...
    .option('-n, --count <n>', 'number of options to generate in interactive mode', (v) => parseInt(v, 10))
    ...
    .option('-f, --filenames-only', 'skip summarisation and use only filenames (faster, less useful messages)')
    ...
    .option('--no-ignore', 'disregard the "ignore" config for this run, so every staged change is ' + 'read')
...
async function runNonInteractive(
  ...
  if (verbose) {
    ...
    for (const [index, summary] of result.summaries.entries()) {
      const label =
        summary.priority === 'low' ? `--- summary ${index + 1} (low priority) ---` : `--- summary ${index + 1} ---`
